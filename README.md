...
터미널이 없는 자리에서 장부를 적고, 고정비를 모으고, 이번 달 실수령액을 본다.
...
기록은 그 폰의 브라우저에만 남는다. 서버로 보내지 않는다. **내보내기·백업**
단추로 나온 csv 는 `at sheet ledger` 가 그대로 읽는 꼴이라, 폰에서 적고 PC 에서
집계하는 식으로 이어 쓴다. 자세한 것은 [mobile/README.md](mobile/README.md).

셈법은 터미널 쪽과 같은 수를 쓴다. `tests/test_mobile.py` 가 최저임금·보험 요율
같은 값이 양쪽에서 어긋나면 걸어 준다 — 한쪽만 고치면 폰과 PC 가 다른 답을 낸다.
