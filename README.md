...
- `ui` 터미널이 불편할 때 쓰는 브라우저 화면 (기능마다 한 화면)
...
at file audit 받은자료 --hidden         # 납품 자료 받자마자
at file docs 제출본 -o 문서목록.xlsx    # 제목·만든 사람·쪽 수를 표로
at file scrub 제출본                     # 미리보기 (무엇이 지워지나)
at file scrub 제출본 --apply             # «…(이름지움).docx» 사본
at file pdf 스캔/ -o 제출본.pdf          # 폴더 안 이미지를 이름 순으로
...
`at file docs` 는 문서를 **열지 않고 속성만** 읽는다(`docProps`). 제목·만든 사람·마지막
저장한 사람·회사·쪽 수가 나온다. 문서를 밖으로 보낼 때 작성자 자리에 사내 계정 이름이
...
**사본**(`…(정보지움).jpg`)을 만든다. 그림 자체는 다시 누르지 않으므로 화질이 그대로다.
**방향(Orientation)은 남긴다** - 그것까지 지우면 폰으로 찍은 사진이 눕혀 보이기 때문이고,
그래도 지우려면 `--all` 을 쓴다.

`at file pdf` 는 사진·스캔 이미지를 **한 장에 하나씩** 담은 PDF 로 묶는다. jpg 는 다시 누르지
않고 그대로 넣어 화질이 그대로고(DCTDecode), png 는 픽셀을 풀어 다시 담는다. 비율은 그대로
두고 쪽 가운데에 놓는다 - 늘려 채우면 스캔한 서류가 찌그러진다. **프로그레시브 jpg 는 넣지
않는다**(PDF 뷰어가 빈 쪽으로 보여 준다) - 대신 어느 파일이 왜 빠졌는지 알려 준다. 인터레이스
png, CMYK jpg 도 같은 방식으로 건너뛴다. 가로로 긴 이미지는 눕혀 담는다(`--no-rotate` 로 끈다).

PDF 는 **속성과 쪽 수만** 본다 - 제목·만든 사람·만든 프로그램·만든 날짜와 쪽 수, 그리고
암호가 걸렸는지. 본문 글자는 꺼내지 않는다(글꼴에 따라 조용히 틀린 글자가 나온다). 압축된
객체 묶음(ObjStm) 안에 든 쪽도 풀어서 세지만, 그래도 못 읽으면 «쪽 수를 읽지 못했습니다» 라고
적는다. 암호가 걸린 파일은 열지 않는다.

`at file scrub` 은 그 이름들을 지운 **사본**을 만든다(`…(이름지움).docx`). 워드·엑셀·슬라이드와 한글(hwpx)을 다루고 PDF 는 건드리지 않는다. 원본은 건드리지
않는다 - 제자리에서 고치면 되돌릴 방법이 없다. 지우는 것은 문서 속성(`docProps`)의 만든
사람·마지막 저장한 사람·회사·관리자뿐이다. **메모·변경 내역·본문에 적힌 이름은 그대로
둔다**(그건 내용이라 손대면 문서가 달라진다). 메모가 든 문서는 그렇다고 따로 알려 준다.
...
`audit` 은 받은 폴더를 한 번에 훑는다(구성, 손볼 이름, 빈 파일, 딸려 온 찌꺼기, 큰 파일,
내용이 같은 파일). 고치지 않는다. 이름은 맥에서 만든 **자모 분리 한글**, 윈도우에서 못 쓰는
글자, 너무 긴 이름, 겹친 공백을 본다 — `at file fixname` 으로 정리할 수 있다. `.DS_Store` 나
`~$문서.xlsx` 같은 것은 «이름 문제» 가 아니라 «찌꺼기» 로 따로 센다. 큰 폴더에서 해시 비교가
부담이면 `--no-dupes` 로 끄고, 껐다는 것을 결과에 적는다.
...
`-o` 로 파일을 내는 명령은 **이미 있는 파일을 말없이 덮지 않는다**. 같은 이름을 다시 주면 «이미 있는 파일입니다» 라고 멈추고, `--overwrite` 를 붙여야
덮어쓴다 — 받은 자료를 정리해 내다가 원본을 지우면 되돌릴 방법이 없다. 제자리에서 고치는
명령(`at doc toc/index --apply`, `at dev mask --in-place`, `at json set --apply`)은 백업을 남기고
`at text undo` 로 되돌린다.
...
안 된다. 값이 너무 다양한 열은 표기 흔들림을 건너뛰고 그렇다고 말한다.
...
**지우지 않는다** — 드문 값이 곧 틀린 값은 아니고, 그 판단은 사람이 한다.
...
보내는 것은 사람이 한 번 더 보고 누른다. 메일 주소로 보이지 않는 행과 첨부를 찾지 못한 행은
만들지 않고 몇 행이 왜 빠졌는지 알려 준다 - 반쯤 채운 초안을 만들어 두면 그대로 나간다.
...
채웠는지 알려 준다. 여섯 자리만 적힌 칸(`900101`)은 1990년인지 2090년인지 정할 근거가 없어
읽지 않는다.

`at sheet ics`읽는 쪽도 남이 만든 파일을 넉넉히 받아들인다. 칸 주소()를 적지 않는 도구가 있는데, 그때는
**나온 차례를 열 자리로** 본다 - A 열로 몰아 넣으면 앞 칸이 조용히 사라진다. 인라인 문자열
()과 서식이 섞인 글자도 이어 붙여 읽고, 수식 셀은 저장된 계산 결과를 읽는다
(결과가 없으면 수식 자체를 글자로 돌려준다).

 는 엑셀로 만든 일정표를 캘린더가 읽는 `.ics` 로 낸다. 시작 칸에 시각이
...
`at text count` 는 **공백 포함**과 **공백 제외** 글자 수를 함께 낸다 - 자소서·과제·기고는
어느 기준인지가 매번 다르고, 하나만 내면 반드시 틀린 쪽을 보게 된다. 원고지 매수는 200자를
한 장으로 센다. 워드(`.docx`)·한글(`.hwpx`) 문서는 문단 글자만 세며(머리글·바닥글은 빠진다)
그렇다고 함께 적는다. `--limit-chars` 를 넘으면 1 로 끝난다.

`at doc todo` 는 회의록에 적어 둔 `- [ ]` 줄을 모아 담당(`@이름`)·기한·절과 함께 한 표로
낸다. **체크 상자가 있는 줄만 센다** - «~하기로 했다» 같은 문장까지 세면 목록을 믿고 쓸 수
없다. 기한은 줄에 적혀 있을 때만 채우고, `3/15` 처럼 해가 없는 표기는 올해로 본 뒤 그렇게
적어 둔다. 기한이 지난 할 일이 있으면 1 로 끝나므로 아침에 한 번 돌려 볼 수 있다.
...
그대로 쓰고, 검증에 실패하면 왜 실패했는지 그대로 옮긴다 — 만료 말고도 이름이 안 맞거나 사슬이
끊긴 경우가 있다. **여기서 통과했다고 모든 브라우저가 믿는다는 뜻은 아니다.**

`at dev doctor` 는 방금 클론한 저장소를 훑는다. 마커 파일(`pyproject.toml`, `package.json`,
`go.mod` …)로 무엇으로 만들어졌는지 보고, 요구 판과 여기 깔린 판, 설치 흔적(`.venv/`,
...
사업자번호는 검증번호까지 맞지만 **실제로 등록된 번호가 아니다**. 주민등록번호는 만들지
않는다 — 형식이 맞는 주민번호를 뿌리는 도구는 만들 이유가 없다.
...
at text wrap README.md -w 80                           # 미리보기
at text wrap docs/ -g '*.md' -w 80 --apply
...
`repeat` 은 문장 단위로 비교하되 공백·문장부호·대소문자 차이는 무시한다. 코드 블록과 표의
줄은 보지 않는다 — 거기서 같은 줄이 나오는 것은 반복이 아니다. 짧은 문장은 자연스럽게
겹치므로 `--min-chars`(기본 12자)로 거른다. 일부러 반복한 문장일 수 있으니 판단은 사람이
한다. 찾은 게 있으면 종료 코드 1이다.

`wrap` 은 **공백에서만** 끊는다. 한국어는 낱말 가운데에서 끊으면 읽기 나빠지기 때문이다.
폭은 글자 수가 아니라 터미널에서 차지하는 칸으로 센다(한글 두 칸). 코드 블록, 표, 목록,
인용, 제목은 건드리지 않는다 — 접으면 마크다운 구조가 깨진다(`--all` 로 포함할 수 있다).
...
사무 문서에 자주 나오는 **외래어 표기**(`워크샵→워크숍`, `스케쥴→스케줄`, `메세지→메시지`,
`버젼→버전`)도 규칙에 있다. 규칙끼리 겹쳐 바른 말을 망가뜨리지 않는지는 시험에서 확인한다.
...
| `at doc stats [경로…]` | 문서 크기 - 글자 수·읽는 시간·표·링크·코드줄·남은 메모 |
| `at doc todo [경로…]` | 문서에 적어 둔 할 일 모으기 (회의록의 `- [ ]` 줄, 담당·기한) |
...
at doc stats docs/                         # 어느 문서가 길고 얼마나 걸리는지
at doc todo 회의록/ --who 홍길동           # 내가 맡은 것만
at doc todo 회의록/ -o 할일.xlsx           # 기한 지난 게 있으면 exit 1
...
화면에 적는다). `<script>` 와 `<style>` 은 통째로 버린다. 깨진 HTML 도 표준 라이브러리
파서가 대충 넘어간다. 웹에서 받아오는 일은 `at dev http` 가 하므로 둘을 이어 쓰면 된다.

`index` 는 문서마다 첫 제목과 첫 문단을 모아 목록을 만든다. 표·인용·코드 블록은 건너뛰고
**진짜 첫 문단**을 찾는다. `-o` 로 준 파일의 `<!-- index -->` 와 `<!-- /index -->` 사이만
갈아 끼우므로, 넣을 자리는 사람이 정한다(`toc` 와 같은 방식이다).
...
`at git mine` 은 **내 커밋만** 날짜별로 모은다(기본은 `git config user.name`, 최근 7일).
주간보고를 쓸 때 여기서 그대로 옮겨 적으라고 만든 것이라 `-o` 로 마크다운 목록을 낼 수 있다.
이름이 안 걸리면 그 기간에 커밋한 사람 목록을 알려 준다 — 이름을 조금 다르게 쓰는 저장소가
많다.
...
| `at sheet audit <파일>` | 받은 표 한 번에 훑기 — 빈 칸·타입 섞임·중복 행·드문 값·개인정보·표기 흔들림 |
...
| `at sheet age <파일> -c <열>` | 생년월일 열에서 만 나이·연령대·성별 열 만들기 |
...
`rename` 은 거래처마다 다른 열 이름을 합치기 전에 맞출 때 쓴다. 앞뒤 공백과 대소문자는
무시하고 찾으므로 `' 수량 '` 과 `Qty` 도 걸린다. **표에 없는 이름은 조용히 넘기지 않고**
알려 주고 종료 코드 1을 돌려준다 — 매핑이 낡았는지 그때 알아야 한다. 바꾸다 이름이 겹치면
뒤에 번호를 붙인다.
...
| `at life annual <입사일>` | 연차 일수 - 입사일 기준 (근로기준법 제60조) |
| `at life severance <입사일> <퇴사일> --pay <3개월임금>` | 퇴직금 계산 (평균임금 기준, 세전) |
...
| `at life rent --deposit <금액>` | 전월세 전환. 보증금 ↔ 월세를 전환율로 계산 |
| `at life time <구간…>` | 근무 시간 계산 (휴게 제외, 자정 넘김), 시각 더하기 |
| `at life won <금액>` | 금액을 한글로 - 읽기용과 계약서용(`일금 …원정`) |
...
at life annual 2023-03-02                  # 오늘 기준 연차 며칠
at life annual 2023-03-02 --table 5        # 앞으로 5년치
at life severance 2021-03-02 2026-09-01 --pay 1500만 --bonus 400만
...
at life rent --deposit 5억 --keep 1억 --rate 5.5   # 반전세로 돌리면 월세 얼마
at life rent --monthly 80만 --deposit 1억 --rate 5.5   # 월세를 보증금으로
at life time 09:00-18:30 --break 60     # 8시간 30분
at life time 22:00~06:00                # 야간 근무도 그대로
at life time 09:00 +3h20m               # 12:20
...
`severance` 는 법정 퇴직금을 **세전**으로 낸다. 평균임금 = 퇴직 전 3개월 임금총액 ÷ 그 기간의
달력 일수, 퇴직금 = 평균임금 × 30일 × (재직일수 ÷ 365). 연간 상여금과 전년도 연차수당은
`3/12` 만 더하고, 통상임금이 평균임금보다 크면 그쪽으로 계산한다. 계속근로 1년 미만이면
법정 퇴직금이 생기지 않는다고 알려 준다. **퇴직소득세는 빼지 않는다** — 근속연수공제가 얽혀
있어 이 자리에서 맞출 수 없다. 퇴직연금(DC)에 든 회사는 운용 결과에 따라 실제 금액이 달라진다.

`annual` 은 **입사일 기준**으로 근로기준법 제60조 그대로 센다. 1년 미만은 한 달 개근마다
하루(최대 11일), 1년부터 15일, 3년째부터 2년마다 하루씩 늘어 최대 25일이다. 회계연도 기준으로
운영하는 회사는 **회사 규정이 우선**이라 계산하지 않고, 출근율 80% 미달·휴직·중도 입퇴사도
반영하지 않는다 — 근태 자료 없이 맞출 수 없는 것을 그럴듯하게 채우면 실제 일수와 다른 답이
나온다. 2월 29일 입사자는 그 날이 없는 해에 3월 1일로 본다.
...
`time` 은 구간을 여러 개 줘도 되고(`09:00-12:00 13:00-18:00`), 끝 시각이 시작보다 이르면
**자정을 넘긴 것**으로 본다(야간 근무). 휴게가 일한 시간보다 길면 0 으로 가리지 않고 음수를
그대로 보여 준다 — 입력이 잘못됐다는 신호다. `--rate` 로 임금을 낼 수 있지만 **연장·야간
가산은 계산하지 않는다**. 가산율은 근로 형태마다 달라서 사람이 적용해야 한다.

`won` 은 읽기용(`백이십오만`)과 계약서용(`일금 일백이십오만원정`)을 함께 낸다. 계약서 표기는
자리를 붙여 쓰고 `일천`처럼 앞의 `일`을 살린다 — 숫자를 덧붙여 고치기 어렵게 하려는 표기다.
메일 머지 틀에서도 `{금액:한글}`, `{금액:계약서}` 로 쓸 수 있어 청구서·견적서를 만들 때
숫자와 한글을 함께 넣을 수 있다.

`rent` 는 **준 전환율로만** 계산한다. 전환율은 계약마다 다르고, 주택임대차보호법의 상한은
그때의 기준금리에 달려 있어서 이 도구가 정할 수 없다. 그래서 상한이 있다는 사실만 알리고
값은 사용자가 넣게 했다. 세금이나 관리비는 보지 않는다.
...
1권으로 잡는다. 출판사마다 다르므로 어림값이다. 기준은 `attools/write/manuscript.py` 상단 상수에서 바꾼다.
상투 표현·부사 목록도 같은 파일의 `CLICHES`, `FILLER_ADVERBS` 에서 고칠 수 있다.

## ui — 브라우저 화면

터미널을 쓰지 않는 사람도 같은 기능을 쓰게 하려고 둔 화면이다. 서버가 아니다 —
`127.0.0.1` 에만 붙고, 실행할 때마다 만든 열쇠(`?t=...`)를 들고 온 요청만 받는다.
다른 웹페이지가 몰래 내 컴퓨터의 파일을 옮기지 못하게 하기 위해서다. 창을 닫고
터미널에서 Ctrl+C 를 누르면 끝난다.

| 명령 | 하는 일 |
| --- | --- |
| `at ui` | 어떤 화면이 있는지 고른다 |
| `at ui 파일정리` | 한 번에 훑기·정리·사진 촬영일별·목록 만들기·중복 모으기·폴더 견주기·첨부 나눠 담기 |
| `at ui 바꾸기` | 여러 파일의 글자·인코딩·줄바꿈을 한꺼번에 |
| `at ui 엑셀` | 다섯 탭 - 훑어보기(한 번에 훑기·점검·드문 값)·고치기(정리·표기 통일·빈 칸·날짜 열·마감일·찾아 바꾸기)·골라내기(집계·그림)·여러 파일(견주기·찾기·취합)·내보내기 전에(가리기·비슷한 값) |
| `at ui 문서` | 점검·용어·그림·목차·표 정렬·HTML/워드→마크다운·여러 문서 합치기·내보내기 |
| `at ui 원고` | 분량·되풀이·인물 등장 점검, 투고본(HTML·EPUB·워드)으로 내보내기 |
| `at ui 글자` | 자판 실수·표기 오류·줄 접기·NFC·표를 마크다운으로·연락처 뽑기 |
| `at ui 단축키` | 프로그램별 단축키를 나란히 놓고 찾는다 |
| `at ui 개발` | 정규식·JWT·시각·cron·가리기·인코딩·키·.env·로그·sqlite·의존성 |
| `at ui json` | 응답 구조·두 응답 비교·표로·타입 코드 |
| `at ui 저장소` | 커밋 전 점검·시크릿·브랜치·충돌·TODO·통계·파일 이력·변경 로그 (읽기만) |
| `at ui 계산` | D-day·더치페이·대출·영업일·적금·전월세·근무시간·연차·퇴직금·단위·세금 |
| `at ui --list` | 화면 목록을 터미널에 찍는다 |
| `at ui --check` | 모든 화면을 브라우저로 열어 자바스크립트 오류를 본다 (오류 1, 못 열면 2), 서버가 화면을 못 내주는 것도 잡는다 |
| `at ui --port 8765` | 포트를 고정한다 (기본은 비어 있는 것 아무거나) |
| `at ui --no-open` | 브라우저를 열지 않고 주소만 찍는다 |

```
$ at ui 파일정리
파일 정리 화면을 띄웠습니다.
  http://127.0.0.1:51234/?t=VQn191d_4VSFSGKaAwBDlA
  이 컴퓨터에서만 열립니다. 주소 끝의 열쇠(t=)까지 있어야 들어갑니다.
  끝내려면 Ctrl+C.
```

화면이 열두 개라 런처 맨 위에 **찾기 상자**를 두었다. «취합», «첨부», «연차» 처럼 하고 싶은 일을
적으면 그 화면만 남는다(화면 이름·별명·한 줄 설명을 함께 본다). 그래도 없으면 터미널에서
`at find <하고싶은일>` 로 명령을 찾는다.

표는 어디서든 오른쪽 위 `csv` 를 눌러 내려받는다(엑셀에서 바로 열리게 BOM 을
붙인다). 화면에서 한 일은 **같은 일을 하는 터미널 명령 한 줄로 함께 보여 준다.** 다음부터는
직접 칠 수 있고, 여러 폴더에 되풀이할 때는 그쪽이 훨씬 빠르다.

CLI 와 같은 규칙을 지킨다. **미리보기를 먼저 보여주고**, 「이대로 옮기기」를
누를 때 계획을 서버에서 다시 세운다 — 화면이 보낸 경로를 그대로 옮기지 않는다.
옮긴 기록은 `~/.attools/journal/` 에 남으므로 화면에서도 터미널에서도
(`at file undo`) 되돌릴 수 있다.

경로 칸 아래의 「찾아보기」로 폴더를 훑어 고를 수 있다. 브라우저는 폴더를 고를
방법을 주지 않으므로(파일 선택 창은 경로를 알려 주지 않는다) 서버가 폴더 안을
보여 주는 식이다. 화면에 넣은 경로는 `~/.attools/ui-recent.json` 에 다섯 개까지
기억해 두었다가 다음에 열 때 채워 준다. 브라우저 저장소가 아니라 파일에 두는 이유는, 실행할
때마다 포트가 달라져 브라우저가 매번 다른 곳으로 보기 때문이다.

엑셀 화면도 같다. **원본은 절대 덮어쓰지 않고** 옆에 «(정리)» 파일을 새로 만든다.
이미 그 이름이 있으면 뒤에 번호를 붙인다.
