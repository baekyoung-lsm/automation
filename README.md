...
`at` 만 치면 갈래와 그 안의 명령 수가, `at sheet` 처럼 갈래만 치면 그 갈래의 명령 목록이
나온다 — argparse 가 내는 사용법 한 줄에 이름 쉰 개가 이어지면 아무도 못 읽는다.
이름을 잘못 쳤을 때도 영어 오류 대신 **비슷한 이름을 짚어 준다**(`at sheet clen` →
«혹시 이것인가요: clean, collect, check»). 명령 자리에 **파일 이름을 치면**(`at 지출.xlsx`)
그 파일로 할 만한 명령을 보여 주고(`at sheet peek 지출.xlsx`), `at help`·`at 도움말`·`at ?` 는
갈래 목록을 낸다 — 처음 쓰는 사람이 제일 먼저 치는 말들이다. 형식을 잘못 준 자리도
**무엇으로 열면 되는지** 알려 준다 — `at sheet peek 발표.pptx` 는 `at sheet from-pptx` 를,
`at doc lint 보고서.docx` 는 `at doc from-docx` 를 가리키고, 워드로 연 파일이 사실 슬라이드나
엑셀이면 그 이름을 짚어 준다(같은 zip 이라 알맹이를 봐야 안다). 원고 자리(`at novel …`)나 로그
자리(`at dev log`)에 문서·표 파일을 줘도 마찬가지다 — 바이트를 글자로 세면 «6매», «88줄» 같은
숫자가 나오는데, **읽은 척하는 것이 제일 나쁘다.**

무엇부터 쳐야 할지 모르겠으면 **`at how`** 가 자주 하는 일 68가지를 명령 줄과 함께 보여 준다
— `at how`(주제 목록), `at how 취합`(그 주제 전부), `at how 개인정보`(말로 찾기), `at how --all`.
거기 적힌 명령은 **하나도 빠짐없이 진짜 파서에 걸어 시험한다**(`tests/test_how.py`) — 안내만
그럴듯하고 안 되는 명령이면 없느니만 못하다. 같은 레시피가 `<명령> --help` 아래 **예시**로도
붙는다(손으로 적어 둔 예시가 있으면 그대로 둔다) — 사용법을 두 군데에 따로 적으면 한쪽만
고쳐지기 때문이다.

명령 이름으로 찾을 때는 `at find <말>` — `at find 중복`, `at find 시간`.
띄어쓰기는 무시하고(`글자수` 로 쳐도 `글자 수` 가 걸린다), 사람이 쓰는 말과 도움말의 말이
다른 자리(`압축`→zip, `맞춤법`→표기 오류, `연락처`→vcf)는 짝을 지어 뒀다.
셸 자동완성은 `eval "$(at completion bash)"` (zsh 도 있다).

- `file` 파일 분류·이름 정리·중복 탐지·변경 감시, PDF 쪽 뽑기·번호·합치기·글자 꺼내기
...
- `sheet` 엑셀·CSV 훑어보기, 검증, 정리, 병합, 비교, 집계, 변환, 메일 머지·라벨·근태
...
**파이썬 3.10 이상만 있으면 된다.** 따로 깔 것도, 인터넷도 필요 없다 —
표준 라이브러리만 쓴다.

**윈도우** — 깃허브에서 `Code` → `Download ZIP` 으로 받아 풀고,
`at.bat` 을 더블클릭한다. 브라우저 화면이 뜬다. 폴더나 파일을 `at.bat` 에
끌어다 놓으면 그것으로 할 만한 명령을 알려 준다. 터미널을 쓸 거면
`at.bat file sweep .` 처럼 그대로 넘기면 된다.

파이썬이 없으면 `at.bat` 이 어디서 받을지 알려 준다. 설치 화면 아래의
**«Add python.exe to PATH»** 를 켜야 한다.

**맥·리눅스** — `git` 으로 받으면 그대로 돈다.
...
ZIP 으로 받으면 **실행 권한이 빠져 있다**(깃허브 ZIP 이 그렇다). 둘 중 하나:

```bash
chmod +x at && ./at --help     # 한 번만 해 두면 된다
python3 at --help              # 또는 그냥 파이썬으로 부른다
```

**어디서나 쓰고 싶으면**

```bash
ln -s "$PWD/at" ~/.local/bin/at   # PATH 에 링크
pip install -e .                  # 또는 패키지로 설치 (at 명령 생성)
python3 build.py                  # 한 파일(dist/at.pyz)로 묶어 나눠 주기
```

폰 화면(`mobile/`)은 파일을 여는 것만으로도 쓸 수 있지만, 홈 화면에 앱으로
깔고 오프라인까지 쓰려면 http 로 받아야 한다 — [폰에 까는 화면](#폰에-까는-화면)
을 보라.
...
| `at file pdfcut <파일>` | PDF 에서 필요한 쪽만 뽑는다. `--drop` 으로 빼기, `--each` 로 한 쪽씩 (돌리기는 `pdfturn`) |
| `at file pdfjoin <파일…>` | 여러 PDF 를 준 차례대로 합친다 (인터넷에 올리지 않고) |
| `at file pdftext <파일\|폴더>` | PDF 에서 글자를 꺼낸다 (찾기·붙여넣기). 폴더면 한꺼번에. 글꼴에 글자 정보가 없으면 그 부분은 빼고 알려 준다 |
| `at file pdfimg <파일>` | PDF 안에 든 **사진·도표를 파일로** 꺼낸다. jpg 는 눌린 그대로 내고, 모르는 방식은 «못 꺼낸 것» 으로 이유와 함께 적는다 |
...
| `at file eml <파일\|폴더>` | 받은 메일(.eml)의 보낸이·제목·시각·첨부를 훑고, `-o` 로 목록을 표로, `--save` 로 첨부를 꺼낸다 |
...
at file pdfblank 스캔본.pdf --only           # 백지로 보이는 쪽만
at file pdfstamp 계약서.pdf --image 도장.png --pages 3 -o 날인본.pdf
...
`at file sweep` 은 **여러 폴더를 한꺼번에** 훑어 «쓰임새» 별로 묶는다 — 다운로드·바탕화면·
문서에 나뉘어 쌓인 것을 한자리에서 본다(폴더를 비우면 홈 아래 흔한 폴더 중 있는 것만 본다).
확장자가 아니라 **무엇에 쓰는 파일인가** 로 나눈다. 같은 `.png` 라도 이름에 «Screenshot»·
«화면 캡처» 가 있으면 스크린샷, `IMG_`·`DSC`·`KakaoTalk_` 이면 사진, `.crdownload` 는
«받다 만 파일» 이다. 이름으로 미루어 짐작한 자리는 `--purpose <쓰임새>` 로 **왜 그렇게
봤는지**를 함께 보여 준다 — 짐작을 확정처럼 적으면 엉뚱한 것을 옮기고도 모른다.
`--to <폴더>` 로 고른 묶음만 한곳에 모으고, 여느 이동과 같이 미리보기가 기본이며
`at file undo` 로 되돌아간다. **권한이 없어 못 들어간 폴더는 개수와 자리를 적는다** —
조용히 빼면 «파일 3개» 가 전부인 줄 알게 된다(없는 폴더·겹치는 폴더도 마찬가지다).
...
`at file docs` 가 보는 PDF 는 **속성과 쪽 수만**이다 - 제목·만든 사람·만든 프로그램·만든
날짜와 쪽 수, 그리고 암호가 걸렸는지(본문 글자는 `at file pdftext` 가 따로 꺼낸다). 쪽 수는
쪽 나무를 걸어가 세고(정규식으로 «/Type /Page» 를 세면 압축 묶음 안의 옛 판까지 세어 부푼다),
그렇게도 못 읽는 파일만 훑어 센 다음 «쪽 수를 읽지 못했습니다» 라고 적는다. 글꼴이 하나도
걸리지 않은 PDF 는 **스캔본으로 보인다**고 알려 준다 - 그런 파일은 글자 검색·복사가 안 되고,
받는 사람이 열어 보고 나서야 알게 되는 것보다 미리 아는 편이 낫다. 암호가 걸린 파일은 열지
않는다.

`at file eml` 은 받은 메일 파일(`.eml`)을 훑는다. 메일을 통째로 내려받거나 전달받으면 `.eml`
로 오는데, 그 안의 첨부를 꺼내려면 메일 앱에 다시 넣어야 했다. 보낸이·제목·받은 시각·첨부 수를
표로 보이고 `--body` 로 본문을, `-o 목록.xlsx` 로 목록을, `--save 폴더 --apply` 로 첨부를 꺼낸다. **읽기만 한다 — 보내지
않는다.** 첨부 이름은 메일에 적힌 값이라 `../` 나 아주 긴 이름이 섞여 오므로 **파일 이름으로
쓸 수 있게 다듬고** 같은 이름은 «(1)» 을 붙인다. 아웃룩의 `.msg` 는 형식이 아예 달라 읽지
못한다(메일 앱에서 `.eml` 로 저장하면 된다). «보낸 사람» 은 메일에 적힌 값 그대로다 — 메일
머리글은 누구나 적을 수 있어 진짜 보낸 사람인지는 알 수 없다.

`at file pdftext` 는 PDF 에서 **글자를 꺼낸다**. 폴더를 주면 그 안의 PDF 를 한꺼번에 훑고,
`-o 폴더` 면 파일마다 «이름.txt» 로 저장한다 — 받은 계약서 스무 개를 한 번에 글자로 만들 때
쓴다. 못 연 파일은 조용히 빼지 않고 표에 까닭과 함께 남긴다. PDF 는 «글자» 가 아니라 «어느 글꼴의 몇 번
글리프» 를 적어 둔 형식이라, 그 번호가 무슨 글자인지는 글꼴이 `ToUnicode` 표를 달고 있을 때만
알 수 있다. **표가 없으면 지어내지 않는다** — 그 글꼴로 찍힌 부분은 빼고, 어느 글꼴이 그랬는지
알려 준다(번호를 아무 글자로나 바꾸면 «꺼냈는데 뒤죽박죽» 이 되어 더 나쁘다). 라틴 글꼴처럼
표가 없어도 인코딩(`WinAnsiEncoding` 등)이 정해진 것은 그대로 읽는다. **스캔한 그림 속 글자는
읽지 못한다** — 글자가 없는 쪽이 있으면 몇 쪽인지와 함께 그렇게 적는다. 줄은 글자를 찍는
**세로 자리(y)가 바뀔 때만** 나눈다 — 낱말마다 자리를 옮겨 찍는 문서가 있어서, 옮길 때마다
나누면 «한 줄에 한 낱말» 이 되어 여러 낱말로는 찾을 수 없다. 같은 줄에서 자리를 옮긴 곳과
`TJ` 로 크게 벌린 자리는 빈칸으로 본다.

`at file pdfimg` 는 PDF 안에 든 **그림을 파일로** 꺼낸다. 보고서에 붙은 도표,
계약서에 붙은 사진을 다시 쓰려고 화면을 찍는 대신 원본 그대로 꺼내는 것이다. **jpg 는
눌린 그대로** 낸다 — 풀었다 다시 누르면 화질만 깎인다. 눌리지 않은 화소는 PNG 로 싸고,
색표(Indexed)는 편다. **CMYK 나 CCITT 처럼 우리가 모르는 것은 짐작해 내지 않고**
«꺼내지 못한 것» 에 쪽·이름·까닭을 적는다 — 조용히 빼면 그림이 몇 장 없는 문서로 보인다.
자료가 모자란 그림도 채워 내지 않는다(채우면 그럴듯한 그림이 나오지만 그 그림은 거짓이다).
기본은 **무엇이 들어 있는지만** 보여 주고, `--apply` 라야 파일을 만든다. 로고·아이콘이
수십 개씩 나오는 문서가 있어 `--min 20KB` 로 거를 수 있고, 걸러 낸 개수는 함께 적는다.

`at file pdfstamp` 은 PDF 에 **도장·서명 그림을 얹는다**. 계약서에 도장을 찍으려고
인쇄해서 찍고 다시 스캔하는 일을 없애려는 것이다. **PNG 의 투명한 자리는 비친다**
(알파를 `SMask` 로 옮긴다) — 알파를 버리면 하얀 네모가 글자를 덮으므로 도장은 배경이 없는
PNG 가 낫다(투명도가 없으면 그렇다고 미리 알려 준다). 크기는 `--width 30`(mm)으로 주고
세로는 비율대로 따라간다. **돌아가 있는 쪽(`/Rotate`)에는 보는 사람 기준으로 맞춰 얹는다** —
그대로 찍으면 종이 밖으로 나간다. 원래 내용 스트림은 건드리지 않고 **뒤에 한 겹 더** 붙이므로
글자와 그림의 화질이 그대로고, 원본 파일도 그대로 둔 채 새 파일을 만든다.

`at file pdfblank` 은 스캔본에서 **빈 쪽**을 찾는다. 양면 스캔을 하면 뒷면 백지가 그대로
섞여 들어오는데, 제출본에서는 그것부터 뺀다. **글자도 그림도 없는 쪽만 «빈 쪽»** 이라고
한다. 스캔한 백지는 «흰 그림 한 장» 으로 들어 있어 그것만으로는 알 수 없어서, 글자가 없고
**그림 용량이 다른 쪽보다 훨씬 작은** 쪽을 «백지일 수 있음» 으로 따로 적는다 — 용량만 보고
짐작한 것이므로 열어 보고 빼라고 함께 적는다. 마지막에 그 쪽을 빼는 `at file pdfcut … --drop`
명령을 그대로 적어 준다. **스캔한 그림 속 글자는 읽지 못하므로** 글자 수가 0이라고 빈 쪽인
것은 아니다.

`at file pdfcut` 과 `at file pdfjoin` 은 쪽을 뽑고 붙인다. 계약서 몇 쪽만 보내거나
스캔한 것을 하나로 묶는 일을 **내 컴퓨터 안에서** 한다 - 그 일 때문에 계약서를 모르는
웹사이트에 올리지 않아도 된다. 눌린 자료(글자·그림·글꼴)는 풀지 않고 **그대로** 옮기므로
화질과 글꼴이 원본 그대로다. 쪽이 걸고 있는 글꼴·그림은 따라온다. **쪽을 골라 내면 책갈피·양식은 따라가지 않고**,
쪽을 다 옮길 때는 목차를 통째로 옮겨 책갈피·양식·쪽 번호 표시까지 살린다.
...
PDF 는 속성(`/Info`)과 XMP 두 군데에 이름이 남아서 둘 다 지운다. 고친 자리만 덧붙이는
방식(증분 갱신)은 쓰지 않는다 - **덧붙이면 옛 이름이 파일 안에 그대로 남아** 꺼내 볼 수 있어서
지운 것이 아니다. 그래서 파일을 다시 쓰고, 다시 쓴 뒤 **한 번 더 열어 이름이 정말 없는지 보고**
남아 있으면 만든 파일을 지운다. 책갈피·양식·쪽 번호 표시·구조 태그는 함께 옮기지만 **전자서명은
무효가 된다**(내용이 같아도 바이트가 달라지기 때문이다). 주석에 적힌 이름은 내용이라 두고,
그런 문서가 있으면 그렇다고 알려 준다.
...
| `at dev db <파일>` | sqlite 파일 훑기 - 표 목록, 열 구성, 외래 키·인덱스, 조회 (읽기 전용) |
...
"무엇이 몇 번 터졌는지"가 바로 보인다. **글자에 번호가 붙은 값**(`user=u23`, `pool3`)과
**단위가 붙은 수**(`took=800ms`)도 같이 묶는다 — 안 묶으면 스무 번 난 같은 오류가 스무 줄로
나와서 정작 무엇이 문제인지 보이지 않는다. 횟수가 같으면 **심각한 것부터** 보여 준다. 스택 트레이스 줄은 앞 항목에 붙이고,
...
**메서드를 안 적는 로그**(`INFO /api/pay took=12ms`)도 경로만으로 묶는다 — 그러지 않으면 전부
`(경로 없음)` 한 줄이 되어 어느 API 가 느린지 안 보인다. 파일 경로(`/var/log/app.log`)와
주소(`https://…`)는 길로 세지 않는다. 그래도 경로를 못 찾은 줄은 버리지 않고 `(경로 없음)`
으로 함께 센다. 시간을 읽은 줄이 절반이 안
되면 그 사실을 알려 준다 — 통계가 일부만 보고 나온 값일 수 있어서다.

백분위는 보간하지 않고 실제 값 중에서 고른다. 값이 몇 개 없을 때 보간하면 로그에 없는
숫자를 지어내게 된다.

레벨이 안 적힌 접근 로그를 `at dev log` 에 주면 묶을 것이 없다고만 나오는데, 그때는
`at dev slow` 로 가라고 알려 준다 — «없습니다» 로 끝나면 다음에 뭘 할지 알 수 없다.
...
이상일 때만 — 두 글자는 한 글자만 달라도 딴 곳이다). 다만 **숫자만 다르면 짝으로 보지
않는다** — `E001` 과 `E002`, `2025년` 과 `2026년` 은 오타가 아니라 다른 번호다(사번 열에서
모든 짝이 «비슷함» 으로 쏟아지던 자리다). 숫자만 빼면 같은 값(`창고1`/`창고2`)도 같은 이유로
짝이 아니고, **날짜와 숫자 칸은 아예 견주지 않는다** — `2026-02-02` 와 `2026-08-20` 은 닮아
보여도 그냥 다른 날이다. 다듬은 이름의 **앞 두 글자가 같은
것끼리만** 견주므로 첫 글자가 다른 오타는 찾지 못한다. 만 행을 전부 견주면 오천만 번을
재야 하기 때문이다. 견주는 것은 행이 아니라 **값**이다 — «개발팀» 300행과 «개 발 팀» 1행이
300줄로 쏟아지면 정작 볼 것을 못 본다. 그래서 짝은 하나로 내고 «4행 외 2» 처럼 몇 행에
있는지를 함께 적는다.
...
`audit` 은 남이 보낸 표를 열어 **무엇부터 봐야 할지** 한 번에 모아 준다(머리글, 빈 칸이 많은
열, 한 열에 섞인 타입, 똑같은 행, 통째로 빈 행, 숫자 열의 드문 값, 개인정보로 보이는 열,
표기 흔들림). 고치지는 않는다.
**무엇을 봤는지와 무엇을 못 봤는지를 함께 적는다** — «볼 만한 곳 없음» 이 «다 봤다» 로 읽히면
안 된다. 값이 너무 다양한 열은 표기 흔들림을 건너뛰고 그렇다고 말한다.

머리글은 특히 자주 어긋난다. 표 위에 **제목 줄이 한 줄 있는 파일**(`2026년 3월 지출 내역`)을
그대로 읽으면 첫 행이 머리글이 되어 그 뒤 모든 명령이 어긋나는데, 그럴 때 «이름이 붙은 열이
하나뿐입니다 — `--header-row 2` 로 다시 읽어 보세요» 라고 알려 준다. 이름이 비어 있어 `열2`
로 붙인 열, 겹쳐서 `이름_2` 로 구분한 열, 앞뒤 빈칸·줄바꿈이 든 이름, 머리글이 전부 숫자인
경우도 함께 짚는다.
...
`gaps` 는 반대로 **없는 것**을 찾는다. 전표 번호가 하나 비었는지, 어느 날 자료가 안
들어왔는지는 있는 자료만 봐서는 보이지 않는다. 번호 열이면 처음부터 끝까지 훑어 빠진 구간을,
날짜 열이면 `--every day|weekday|month` 간격으로 빠진 날을 묶어서 보여 준다. **번호도 날짜도
아닌 칸은 조용히 버리지 않고** 몇 개였는지와 예시를 함께 적는다(«미정», «취소분» 같은 칸이
많으면 애초에 셀 수 없는 열이다). `--holidays` 를 주면 공휴일은 빠진 것으로 세지 않는데,
설날·추석은 음력이라 자동으로 넣지 못하므로 그렇다고 말해 준다. 빠진 것이 있으면 exit 1 이라
마감 점검을 스크립트로 걸어 둘 수 있다.

엑셀 파일에 **시트가 여럿일 때**, 한 시트만 고쳐 `-o` 로 내면 결과 파일에는 그 시트만
담긴다(원본은 그대로다). 그렇다고 **알려 주고**, `--keep-sheets` 를 주면 나머지 시트를 원래
차례대로 함께 옮긴다. `clean`, `format`, `mask`, `dedupe`, `fx`, `rename`, `expand`,
`combine`, `where`, `sort`, `cut` 같은 «한 시트를 손보는» 명령에 있다.
...
`at sheet pivot` 은 값이 **글자로 들어온 숫자**(`1,350,000`, `870000원`)도 읽어서 센다 —
엑셀에서 서식만 숫자처럼 보이는 칸이 흔한데, 그걸 버리면 **합계가 조용히 적어지고 표는
멀쩡히 만들어진다**. 그래도 읽지 못한 칸은 몇 행인지 적는다. 숫자와 글자가 섞인 열에
`--agg min/max` 를 주면 글자로 견준다(그냥 견주면 파이썬이 터진다). `--rows`·`--cols`·
`--values` 는 홑수(`--row`, `--col`, `--value`)로도 받는다 — 홑수로 치면 «ambiguous option»
이 뜨던 자리다.

`at sheet audit` 과 `at sheet merge` 는 **합계 줄**을 짚는다. 부서에서 받은 표 맨 아래에
«합계» 행이 붙어 오는 일이 흔한데, 그대로 합치거나 집계하면 **두 번 세면서도 표는 멀쩡히
만들어진다**. 첫 칸이 합계·소계·총계·Total 인 행을 몇 행인지와 함께 적는다(«합계산업» 처럼
그 말로 시작하는 이름은 세지 않는다). 지우지는 않는다 — 뺄지 말지는 사람이 정한다.
`audit` 은 한 걸음 더 가서 **그 합계가 맞는지도** 본다(`at sheet total --check` 와 같은 셈).
행을 끼워 넣고 합계 식을 안 고친 표가 흔한데, 훑어보는 자리에서 바로 걸린다.

`at sheet audit` 은 «엑셀이 수식으로 읽을 칸» 도 알려 준다. 남이 보낸 표에 `=`·`+`·`-`·`@`
로 시작하는 **글자**가 들어 있으면 엑셀에서 열 때 수식으로 실행될 수 있다(CSV 주입). 값을
고치지는 않는다 - 고치는 것이 맞는지는 사람이 정할 일이라 어디인지만 알려 준다. 숫자로 읽힌
`-5` 는 수라서 세지 않는다.

«내보낼 때 걸릴 값» 도 함께 본다 — **제어 문자**가 든 칸(엑셀·워드로 낼 때 빠진다)과 **엑셀 한 칸
한도(32,767자)를 넘는 값**(xlsx 로 내면 잘린다)이다. 파일을 만든 뒤에 알면 늦다.

`at sheet dedupe` 는 키가 같은 행 중 하나만 남긴다. 취합한 명단에서 사번이 같은 행을
추리고 «수정일이 가장 늦은 것» 만 남기는 식이다. 두 가지를 조심한다. **키가 빈 행은 서로
묶지 않는다** — 빈 값끼리 한 묶음으로 보면 사번을 아직 안 적은 서로 다른 사람 둘이 한
사람이 되어 말없이 사라진다. **`--by` 열이 비었거나 견줄 수 없는 값이면 그 행은 이기지
않는다** — «최신 것만 남겨라» 라고 했는데 날짜가 빈 행이 남으면 시킨 것과 반대다(그런 행이
몇 행인지 적는다). 날짜는 글자로 와도 날짜로 읽고(`2024.3.2`), 한 열에 숫자와 글자가 섞여
있으면 글자로 견준다. **지운 행은 화면에 보여 주고** `--dropped` 로 파일에 남긴다 — 지운
것을 못 보면 되돌릴 수도 없다.

`at doc form` 은 **워드·한글 양식**에 값을 채운다. 위촉장·수료증·공문처럼 기관 서식이 든
`.docx`·`.hwpx` 를 그대로 두고 `{이름}` 자리만 갈아 끼운다 — 원본 zip 을 통째로 베끼고 글자가 든 xml
...
백 장을 만든 뒤에 알면 늦다(`--force` 로 넘길 수 있다). 파일 이름이 겹치거나 이름 칸이 빈
줄이 있으면 하나도 만들지 않는다 — 그대로 만들면 `.docx` 같은 **이름 없는 숨김 파일**이 되어
한 장이 없어진 줄도 모른다.

한글(`.hwpx`)도 같은 규칙으로 채운다. 두 형식 모두 zip 안의 XML 이고 문단은 `p`, 글자는 `t`
태그라 채우는 코드는 한 벌뿐이다(`attools/slots.py`). hwpx 는 맨 앞의 눌리지 않은 `mimetype`
조각까지 그대로 두고 쓴다 — 그것이 어긋나면 한글이 파일을 열지 못한다. **옛 `.hwp`(이진
형식)는 읽지 못한다** — 한글에서 「hwpx로 저장」을 한 번 거쳐야 한다.
...
글꼴·테두리·수식·그림·인쇄 설정이 그대로 남는다 — 새로 만들어 담으면 그게 다 날아간다.
값을 넣은 칸에 수식이 있었으면 그 수식은 지운다(값과 수식이 함께 남으면 엑셀이 어느 쪽을
보여 줄지 알 수 없다). **원본 양식은 건드리지 않고**, 여러 장을 만들 때는 `--apply` 를 붙여야
실제로 만든다. 파일 이름이 서로 겹치면 하나도 만들지 않고 멈춘다 — 덮어쓰면 앞엣것이
사라진다.

`at sheet mask --auto` 는 가릴 열을 짐작한다. 주민번호·전화·이메일은 **값 꼴**로 찾고(그 꼴인
값이 절반을 넘는 열만 — 비고란에 전화번호 한 줄 있다고 비고를 통째로 가리면 표를 못 쓴다),
이름·주소·계좌는 값만 봐서 알 수 없어 **열 이름**으로 찾는다. 무엇을 왜 골랐는지 먼저 보여
주고 가린다 — 짐작이므로 빠진 것이 있으면 `--name` 처럼 직접 주면 된다. 폴더를 주면 안의
xlsx·csv 를 전부 가려 `-o` 폴더에 같은 이름으로 낸다(`--apply` 라야 실제로 만든다).
**원본은 건드리지 않는다.** 한 파일이라도 가릴 열을 못 고르면 아무것도 만들지 않는다 —
반쯤 가린 폴더를 보내는 것이 제일 나쁘다.
...
더 본다 — 마지막 줄은 보통 총계라서다. `80,000` 처럼 글자로 든 숫자도 읽고(버리면 합이 조용히
적어진다), 숫자로 못 읽은 칸은 몇 개인지 따로 알린다. 원 단위 절사 때문에 몇 원 어긋나는
표는 `--tol 1` 로 넘긴다. 안 맞는 곳이 있으면 종료 코드 1 이다.
...
받은 엑셀은 첫 시트가 «안내»·«표지» 인 일이 흔하다. **자료가 한 줄도 없는 첫 시트는 건너뛰고
자료가 든 시트를 읽는다** — 그것을 자료로 읽으면 그 파일은 통째로 0행이 되어 **말없이 빠진다**
(어느 시트를 읽었는지는 표의 «시트» 칸에 적힌다). 기준도 자료가 있는 구성 중에서 고른다.
머리글 줄을 잘못 잡은 파일은 «열이 다른 것» 이 아니라 «읽기 시작한 줄이 다른 것» 이므로
따로 모아 `--header-row` 를 안내한다.
...
폴더에 이미 있는 파일은 **말없이 덮지 않는다** — 사람이 고쳐 둔 초안이 사라져도 «몇 건
만들었습니다» 만 보고는 모른다(`--overwrite` 로 덮어쓴다). 칸 안에 줄바꿈이 있으면 제목·받는
사람 자리에서는 **한 줄로 만든다** — 머리글에 줄바꿈이 들어가면
메일 라이브러리가 통째로 거절해 한 건도 못 만들고, 그 자리가 다른 머리글을 끼워 넣는 구멍이 된다.
자리표시자에는 **파이썬 서식**을 그대로 쓸 수 있다(`fill` 도 같다). 금액은 `{금액:,}` 으로
`1,250,000` 이 되고, 번호는 `{번호:03d}` 로 `007` 이 된다 — 표에서 숫자로 읽힌 값은 그냥
쓰면 `1250000` 으로 나오므로 돈을 적는 자리에는 `:,` 를 붙이는 편이 낫다. 서식 없이 큰 숫자
열을 넣은 자리가 있으면 **만들기 전에 알려 준다** — 안내문이 나간 뒤에 알면 늦다.

`at life hourly` 는 월 통상임금에서 **통상시급**과 연장·야간·휴일 가산 단가를 낸다
(근로기준법 제56조: 연장·야간 50%, 휴일 8시간까지 50%, 넘는 시간 100%). 한 달 소정근로시간은
209시간(주 40시간 + 주휴 8시간 환산)으로 보되 **법이 정한 값이 아니라 흔한 값**이라고 적고
`--hours` 로 바꿀 수 있게 했다. 야간은 연장과 겹치는 일이 많아 **가산분(50%)만** 센다.
통상임금에 무엇이 들어가는지는 회사 규정·판례에 따라 다르고, 5인 미만 사업장은 가산 규정이
적용되지 않는다는 것도 함께 적는다.

`at life weekly` 는 **주휴수당**을 낸다(근로기준법 제55조·시행령 제30조).
`(1주 소정근로시간 ÷ 40) × 8 × 시급` 이고, 40시간을 넘겨 일해도 주휴는 8시간분까지다.
1주 소정근로시간이 **15시간 미만이면 주휴가 없다**고 분명히 말한다. **계산식을 숫자까지 넣어
그대로 보여 주므로** 값이 이상하면 어디가 틀렸는지 바로 보인다. 한 달 어림값은 4.345주로
환산한 것이라 달마다 다르고, 개근이 조건이라 결근한 주에는 나오지 않는다는 것도 적는다.

`at sheet worktime` 은 출근·퇴근 열에서 하루 근무 시간과 주별 합계를 낸다. 휴게는
**근로기준법 제54조의 최소 시간**(4시간이면 30분, 8시간이면 1시간)을 빼고, 회사가 다르면
`--rest 분`으로 준다. 퇴근이 출근보다 이르면 **자정을 넘긴 것**으로 보고 그렇게 표시한다. `--hourly 시급` 을 주면 **기본임금·연장가산·야간가산·일당** 열까지 붙인다 — 연장은
«하루 8시간 초과» 로만 세고(주 40시간 초과는 소정근로시간을 몰라 세지 않는다), 야간은 22~06
사이에 자리에 있던 시간을 실근무 이내로 잘라 센다. 휴일근로와 주휴수당은 세지 않고, 5인 미만
사업장은 가산 규정이 적용되지 않는다는 것도 함께 적는다.
시각은 `09:00`, `9시 30분`, `18시` 말고 **`오전 9시`·`오후 6:30`·`6 PM`** 도 읽는다 — 근태표는
사람이 손으로 적는 칸이라 이렇게 적힌 것이 흔한데, 못 읽으면 그 날이 통째로 빠진 채 집계된다.
표시가 없는 맨숫자(`9`)는 시각인지 알 수 없어 읽지 않는다. 시각을 못 읽은 행은 0으로 채우지
않고 비워 둔 뒤 몇 행인지 알려 준다. `--hourly` 를 안 주면 시간만 세고 임금은 셈하지 않는다 —
임금은 회사 규정과 얽혀 있다.
...
그때는 **나온 차례를 열 자리로** 본다 - A 열로 몰아 넣으면 앞 칸이 조용히 사라진다.
행도 마찬가지다. **빈 행은 파일에 아예 안 적히므로** 나온 차례대로 쌓으면 그만큼 밀려서,
도구가 «3행» 이라고 알려 준 자리가 엑셀에서는 4행이 된다 - 행 번호(`r="3"`)를 보고 그 자리에
놓고 사이는 빈 행으로 채운다(행 차례가 뒤죽박죽인 파일도 번호대로 놓는다). 인라인
문자열(`inlineStr`)과 서식이 섞인 글자도 이어 붙여 읽고, 수식 셀은 저장된 계산 결과를 읽는다
(결과가 없으면 수식 자체를 글자로 돌려준다).
옛 맥 엑셀로 만든 파일(`date1904`)은 기준일이 달라 그대로 읽으면 **모든 날짜가 4년 하고 하루씩
앞당겨진다** - 그것도 보고 읽는다. csv 는 무엇으로 칸을 나눴는지(`,` `;` 탭 `|`) 첫 줄들을 보고
고른다. 나라 설정에 따라 엑셀이 세미콜론으로 내보내는데, 쉼표로만 읽으면 한 줄이 통째로 한
칸이 되고 표는 «열리기» 때문에 틀린 줄도 모른다. 반대로 **머리글 줄이 안 나뉘는 글자는
구분자로 보지 않는다** - 한 열짜리 메모 파일의 «가;나;다» 를 세 칸으로 쪼개면 머리글 너비에
맞춰 잘려 뒤가 통째로 사라진다. **머리글보다 긴 행**이 있으면 그만큼 열을 늘려
(`열3`, `열4`) 값을 살린다 - 머리글 칸을 하나 지운 파일이나 따옴표를 빠뜨린 csv 에서 흔한데,
잘라 버리면 표는 그대로 열리고 값만 없어진다. 끝의 빈 칸(`1,2,`)으로는 열을 만들지 않는다.
...
폴더를 훑을 때는 마크다운만 보는데, **회의록은 `.txt` 로 적는 사람이 많아** `--txt` 를 주면
그것도 함께 본다(안 줬을 때는 «.txt 도 보려면 --txt» 라고 알려 준다 — 조용히 빼면 할 일이
없는 줄 알게 된다).
...
받은 문서는 한 겹 더 싸여 있는 일이 많다. **내용 컨트롤**(`w:sdt`, 양식 문서에 흔하다)이나
수정 표시로 감싼 문단도 찾아 읽는다 - 바로 아래 자식만 보면 그런 문단이 통째로 빠지는데,
문서는 «옮겨졌다» 고 나온다. 링크 글자와 수정 표시로 넣은 글자는 살리고, 지운 것으로 표시된
글자는 넣지 않는다.

이름만 `.csv` 인 파일도 흔하다. 알맹이가 zip(엑셀·워드·한글)이거나 옛 OLE 파일이면 **글자로
읽히더라도 표로 읽지 않는다** — 깨진 글자가 표 한 장으로 나오면 «열렸으니 맞겠지» 하게 된다.
아는 인코딩(`utf-8`, `cp949`, `euc-kr`, `utf-16`)으로 못 여는 파일도 그렇게 알린다.

**암호가 걸린 파일**(엑셀·워드·한글)은 겉은 zip 인데 알맹이가 `EncryptedPackage` 하나뿐이라
«문서가 아니다» 로 보이기 쉽다 — 그렇게 알리지 않고 «암호를 풀고 저장한 뒤에 다시 하세요» 로
알린다. 이름만 `.xlsx` 인 옛 `.xls` 와 암호 건 엑셀은 겉모습(OLE 통)이 같아 가릴 수 없으므로
둘 다 적어 준다 — 한쪽으로 단정하면 엉뚱한 데를 고치게 된다.

`at doc from-pptx` 는 받은 발표자료(`.pptx`)에서 **슬라이드의 글자와 표만** 꺼낸다. 슬라이드
하나가 `## 1. 첫 줄` 로 시작하는 절이 된다 — 첫 문단을 제목으로 올린다. 그림·도형 모양·
애니메이션은 옮기지 않고, 발표자 노트는 `--notes` 를 붙일 때만 옮긴다. 차례는 파일
이름(`slide1`, `slide2`)이 아니라 **발표 문서에 적힌 차례**(`p:sldIdLst`)를 따른다 — 슬라이드를
옮겨 붙인 파일은 이름 순서와 보이는 차례가 다르다. `at text find --docx` 와 `at text count` 도
같은 방법으로 발표자료 안을 본다. 표만 필요하면 `at sheet from-pptx` 가 슬라이드의 표를
엑셀·csv 로 낸다.
...
`--docx` 는 **PDF·발표자료(pptx)·받은 메일(eml)도** 함께 본다 — 받은 계약서·공문이 PDF 인 일이 많아서다. 글꼴에 글자 정보가
...
응답 본문은 `content` 에 적어 둔 **`example`·`examples` 를 스키마보다 먼저** 쓴다 — 사람이 손으로
적어 둔 값이 지어낸 값보다 진짜에 가깝다. 스키마만 있으면 형식에 맞는 값을 채운다.
...
알려 준다 — 앞 20행만 보고 전부라고 믿으면 안 된다. `--table` 로 볼 때는 **외래 키**(`고객id ->
고객.id`)와 **인덱스**도 함께 적는다 — 처음 보는 DB 에서는 표끼리 어떻게 이어지는지가 열 이름
보다 먼저 궁금하다.
...
| 명령 | 하는 일 |
| --- | --- |
| `at text pick <경로…>` | 이메일·전화·사업자번호·금액·날짜·주소를 뽑아 표로 (정규식 없이). `--docx` 로 워드·한글·PDF 안까지 |
| `at text privacy [경로…]` | 폴더 안 문서에 **개인정보**가 들어 있는지 훑는다 (주민등록번호·카드·계좌·여권·연락처). 워드·한글·PDF·슬라이드·엑셀 안까지, 찾은 값은 가려서 보여준다 |
| `at text kbd <글>` | 한/영 자판을 잘못 눌러 깨진 글 되살리기 (`dkssud` → `안녕`, 그 반대도) |
| `at text find <찾을것> [경로]` | 찾기만 한다 (고치지 않음). `-C` 문맥 줄, `--count` 파일별 건수, `--files` 파일 이름만, `--docx` 워드·한글·PDF·슬라이드·메일 안까지 |
...
at text privacy 보낼폴더/                # 밖으로 보내기 전 개인정보 점검
at text privacy 인사자료/ --only 주민등록번호,카드번호 --detail
at text privacy 공유폴더/ -o 점검.csv    # 가린 값으로 점검 기록 남기기
...
`privacy` 는 **밖으로 보내기 전** 한 번 돌려 보는 자리다. 폴더 안의 워드·한글(hwpx)·
PDF·슬라이드·엑셀·메일·글 파일을 열어 주민등록번호·카드번호·계좌·여권·연락처가 들어
있는지 찾는다. 파일은 **고치지 않는다**(읽기만 한다). 찾은 값은 `900101-1******` 처럼
가려서 보여 준다 — 점검하겠다고 주민번호를 터미널에 그대로 찍으면 점검이 새 유출이 된다.

오탐을 줄이려고 검사식을 쓴다. 주민등록번호는 앞 여섯 자리가 실제 날짜여야 하고,
카드번호는 Luhn 검사식을 통과해야 센다. 계좌·여권번호는 같은 줄에 «계좌»·«입금»·«여권»
같은 말이 있을 때만 `짐작` 으로 올린다 — 아무 숫자나 계좌로 세면 표를 못 쓰게 된다.
주민등록번호의 `확인` 은 검증번호까지 맞았다는 뜻이고 `형식` 은 형식만 맞았다는 뜻이다.
2020년 10월부터 주는 번호는 뒤 여섯 자리가 임의번호라 검증번호가 없어 `형식` 으로 나온다
— 그래도 주민등록번호다.

찾은 것이 있으면 종료 코드 1 이라 «보내기 전 점검» 을 스크립트에 걸 수 있다. 못 읽은
파일은 따로 알린다. 스캔한 이미지처럼 글자가 없는 파일은 찾지 못하므로 «없음» 이
«안전» 은 아니다.
...
`at novel rename` 은 인물 이름을 바꾸면서 **뒤에 붙은 조사까지** 새 이름에 맞춘다.
`리안은` → `세하는`, `리안이` → `세하가`, `리안으로` → `세하로`(받침이 ㄹ 이면 `로`).
이름만 바꾸면 `세하은` 이 되어 원고를 처음부터 다시 훑게 된다. 이름 뒤가 조사도 아니고
띄어쓰기도 아니면(`리안느`) 다른 낱말로 보고 건드리지 않는다. 받침 있는 이름 뒤의 `이`
(`민준이는`)는 **조사가 아니라 부르는 말**로 본다 — 조사로 보면 `지호가는` 이 되어 문장이
깨진다. 새 이름에 받침이 없으면 그 `이` 도 떨어진다(`민준이는` → `지호는`, `지훈이는`).
미리보기와 실제로 바꾸는 자리는 같은 함수(`names.rename_hits`)를 쓴다 — 갈리면 되돌리기가
있어도 소용이 없다. 기본은 미리보기이고 `--apply` 로 고친 파일은 백업이 남는다(`at text undo`).
...
| `at doc todo [경로…]` | 문서에 적어 둔 할 일 모으기 (회의록의 `- [ ]` 줄, 담당·기한). `--txt` 로 .txt 회의록도 |
...
| `at doc form <양식.docx\|.hwpx>` | 서식이 든 **워드 양식**에 `{이름}` 자리를 채워 새 파일로. `--data` 로 명단을 주면 **한 행에 한 장** (위촉장·공문·계약서) |
...
표는 **`colspan`·`rowspan` 을 펴서** 옮긴다 — 한 칸으로 세면 열이 밀려 뒤 열의 값이 통째로
어긋난다. 세로로 병합한 칸은 아래 줄에도 값을 채우고(그 값이 그 줄에도 걸린 것이다), 가로로
병합한 칸은 **머리글일 때만** 되풀이한다 — 자료 칸에서 되풀이하면 없던 값이 생겨 합계가 는다.
`1분기 / 1월·2월` 처럼 머리글이 두 줄인 표는 한 줄로 합친다(`1분기 1월`) — 마크다운 표는
머리글이 한 줄뿐이라 그대로 두면 구분줄이 가운데 끼어 표가 깨진다. `th` 가 없는 표는 첫 줄을
머리글로 놓고 그렇게 했다고 주석으로 적어 둔다.
...
쓸모가 없기 때문이다. 파이썬 파일은 `tokenize` 로 **진짜 주석만** 가려낸다 — 여러 줄 문자열
안에 `# TODO` 가 든 시험 자료가 할 일 목록을 채우는 일을 막는다(문법이 깨진 파일은 줄 단위
규칙으로 돌아간다). `TODO(이름)` 이나 `TODO @이름` 으로 적힌 담당자가 있으면 그것을
...
한글이 깨지지 않도록 UTF-8 BOM 을 붙인다. xlsx 로 쓸 때 **한 칸 한도(32,767자)**를 넘는 글은
자르고 «잘림, 원래 N자» 라고 칸 안에 적는다 — 넘겨 쓰면 엑셀이 «파일이 손상됐다» 며 제 마음대로
자르는데, 그렇게 잘린 것은 알아챌 방법이 없다. 한 장의 **행 한도(1,048,576)**나 **열
한도(16,384)**를 넘으면 아예 저장하지 않고 `at sheet split` 을 알려 준다 — 못 여는 파일을
만들어 주는 것보다 낫다. 시트 이름은 금지 문자(`/ \ : * ? [ ]`)를 바꾸고 31자로 자른 뒤
**겹치면 번호를 붙인다** — 같은 이름이 둘이면 엑셀이 파일 자체를 열지 못한다.
`nan`·`inf` 는 숫자 칸에 그대로 못 넣으므로 엑셀이 쓰는 오류 값(`#NUM!`)으로 적고,
**1900년 3월 1일 앞의 날짜**는 글자로 적는다 — 엑셀은 없는 날(1900-02-29)을 세는 버릇이 있어
그 앞의 날짜가 하루씩 어긋난다. 어긋난 날짜를 그럴듯하게 넣는 것보다 낫다.
**제어 문자**(PDF·로그에서 옮겨 온 글에 섞여 온다)는 xlsx·워드·SVG·EPUB·ics 로 낼 때 빼고
쓴다 — XML 이 담지 못하는 글자라 하나만 섞여도 파일이 통째로 안 열린다(EPUB 은 리더가 XML
파서로 읽어 그 화가 안 열리고, 캘린더는 앱마다 다르게 깨진다).
...
| `at sheet mask <파일\|폴더>` | 개인정보를 가린 사본 (이름·전화·이메일·주민번호·계좌·주소). `--auto` 로 가릴 열을 짐작, **폴더를 주면 통째로** |
| `at sheet clean <파일>` | 공백·전각 공백 정리, `"1,234원"` → 숫자, `2024.01.05` → 날짜, 빈 행·열·중복 행 제거 |
| `at sheet merge <파일들\|폴더>` | 월별·부서별로 쪼개진 파일을 세로로 합치고 출처 열을 붙인다. 폴더를 주면 그 안의 xlsx·csv 를 이름 순으로 |
| `at sheet form <양식.xlsx>` | 양식 파일의 칸에 값을 채워 새 파일로 (collect 의 반대). `--data` 로 표를 주면 **행마다 파일 하나**. 서식·수식·그림은 그대로 |
...
| `at sheet pivot <파일> --rows <열>` | 그룹별 합계·평균·건수, `--cols` 로 교차표 (화면에는 앞쪽 열과 합계만·자릿점을 넣어, 파일에는 숫자 그대로) |
...
| `at sheet total <파일>` | 맨 아래에 합계 줄을 붙인다 (`--kind avg\|count`). 화면에서는 가운데를 접고 **합계 줄을 반드시 보여 준다**. `--check` 면 붙이지 않고 **이미 적힌 합계·소계가 맞는지 검산**한다 (안 맞으면 exit 1) |
...
at sheet mask 보낼폴더 --auto -o 가린것 --apply    # 보내기 전 통째로
at sheet clean 원본.csv --dedupe -o 정리본.xlsx
at sheet merge 2026-*.csv -o 통합.xlsx
at sheet merge 월별자료/ -o 통합.xlsx          # 폴더째 주면 이름 순으로
at sheet collect 부서제출/ --cell B3=담당자 --cell C7=금액 -o 취합.xlsx
at sheet forms 부서제출/                     # 합치기 전에 서식부터
at sheet form 견적서양식.xlsx --set B3=한빛상사 -o 견적서.xlsx
at sheet form 견적서양식.xlsx --data 거래처.xlsx --cell B3=업체 --cell C7=금액 \
  --name '{업체}_견적서.xlsx' -o 보낼것 --apply      # 거래처마다 한 장
...
at sheet total 받은정산서.xlsx --check        # 적힌 합계가 맞나 (--tol 1 로 절사 허용)
...
`to-json`·`to-sql` 은 `-o` 가 없으면 결과를 그대로 흘려보낸다(`| jq`, `| psql`). 다만
**화면일 때는 앞 20줄만** 보이고 몇 줄이 더 있는지 적는다 — 3만 행짜리 표는 JSON 으로 20만
줄이라, 그대로 쏟으면 앞서 본 것까지 스크롤 밖으로 밀려난다. 관으로 넘길 때는 전부 나간다.

`from-json` 은 객체 배열을 표로 편다. 열은 모든 원소의 키 합집합이라 어떤 원소에만 있는
키도 빠지지 않고, 없는 값은 빈 칸이 된다. 중첩된 객체는 `meta.부서` 처럼 펴고, 정한 깊이를
넘거나 배열이면 JSON 글자로 남긴다. `--path` 를 생략하면 **한두 겹 안에 들어 있는 배열까지
찾아** 가장 큰 것을 쓰고(`data.items` 처럼 API 응답에 흔한 자리), 어느 것을 썼는지 적는다.
크기가 같은 후보가 여럿이면 고르지 않고 후보를 보여 준다 — 잘못 고르면 엉뚱한 표가 나온다.

`--format` 은 국내에서 자주 쓰는 형식을 본다 — `사업자번호`(국세청 검증번호 계산),
`휴대폰`, `전화번호`, `우편번호`(5자리), `이메일`. 사업자번호는 **규칙에 맞는 번호인지만**
확인한다. 실제로 등록된 사업자인지는 알 수 없다 — 오타를 잡는 용도다. 걸린 규칙 이름에
«검증번호까지 봅니다» 를 붙여 둔다 — `123-45-67890` 처럼 **꼴은 맞는데** 걸리는 값이 있어,
그 말이 없으면 왜 걸렸는지 알 수 없다.
...
파일 이름이 겹치면 **덮어쓰지 않고 «(1)» 을 붙이고** 몇 건이 그랬는지 알려 준다 — 같은 이름이
둘이면 한 건이 통째로 사라지는데, «2건 만들었습니다» 만 보고는 알 수 없다. 이름이 길면
파일 이름 한도(255바이트, 한글 여든 자 남짓)에 맞춰 자른다.
...
`--header-row` 는 **엑셀에서 본 그 줄 번호**다 — 빈 줄이 섞여 있어도 번호가 밀리지 않고,
그 자리가 빈 줄이면 다음 줄로 내려간다. (표 위에 제목 줄이 두어 줄 있는 파일이 흔한데, 빈 줄을
먼저 버리고 세면 «4행» 이 3행이 되어 엉뚱한 줄을 머리글로 잡는다.)

`forms` 는 합치기 **전에** 합쳐 둔 칸이 있는 파일도 함께 알려 준다 — 합친 뒤에는 그 빈 칸이
어느 파일에서 왔는지 알 수 없다. 합계 줄은 맨 앞 칸만 보지 않고 **앞쪽 몇 칸**을 본다(합치면서
붙는 「출처」 열 뒤에 숨은 합계 줄을 놓치면 집계가 두 번 세어진다).

`--unmerge` 는 **합쳐 둔 칸**을 엑셀 화면에 보이는 대로 채워 읽는다. 엑셀에서 칸을 합치면
값은 왼쪽 위 한 칸에만 들어 있어서, 그대로 읽으면 부서 이름이 세 줄 중 한 줄에만 있는 표가
된다 — 그 상태로 집계하면 **소리 없이 다른 답**이 나온다(영업1팀 합계가 100만 잡히고, 나머지는
이름 없는 그룹으로 빠진다). 그래서 합쳐 둔 칸이 있으면 세어서 **늘 알려 주고**, 채우는 것은
`--unmerge` 를 붙일 때만 한다. 원본 파일은 어느 쪽이든 건드리지 않는다. `at sheet collect`
(양식 취합)는 합쳐진 칸을 가리켰을 때 값이 든 칸을 **알아서 따라간다** — 양식은 「담당자」 칸을
`B3:D3` 으로 합쳐 두는 일이 흔한데, `C3` 을 집으면 그 파일만 빠뜨린 것처럼 보이기 때문이다.

날짜는 `2026-03-04`, `2026.3.4`, `20260304` 말고 **`2026년 3월 4일`** 과 **뒤에 붙은 요일**
(`2026-03-04(수)`, `2026.3.4 수`)도 읽는다 — 공문서와 일정표에 흔한 꼴인데, 글자로 두면 그 열만
정렬도 집계도 안 된다. 요일은 날짜에서 다시 구할 수 있어 떼고 본다. 해가 없는 `3월 4일` 은
읽지 않는다 — 올해로 정하면 지어내는 것이 된다.

쉼표는 **세 자리마다 끊었을 때만** 천 단위 구분으로 본다 — `1,2,3` 이나 `3,4` 는 번호를
나열한 것이라 `123`, `34` 로 바꾸면 없는 값이 생긴다.
...
at keys --export 빈칸.csv --gaps  # 빈 칸을 표로 내보내 엑셀에서 채우기
at keys --read 빈칸.csv --apply   # 채워 온 표를 한꺼번에 넣기
...
칸이 여럿이면 **표로 내보내 엑셀에서 채우고 한꺼번에 넣는** 쪽이 빠르다.

```bash
at keys --export 빈칸.csv --gaps   # ? 로 남은 칸만 표로
at keys --read 빈칸.csv            # 채워 온 표 - 미리보기
at keys --read 빈칸.csv --apply    # 실제로 넣기
```

`갈래`·`기능`·`앱` 칸은 그대로 두고 **«채울 값» 칸에만** 적으면 된다(`없음` 도 그대로
적는다). 빈 칸으로 두면 그 칸은 `?` 로 남는다 — **아는 것만 적으라는 뜻이다.** 갈래나
...
at life insure 4800만 --annual --tax 84850   # 연봉으로, 실수령액까지
...
`insure` 는 월급에서 4대보험이 얼마나 빠지는지 센다. 2026년 요율 기준이다 —
국민연금 9.5%, 건강보험 7.19%, 장기요양 0.9448%(소득 대비), 고용보험 1.8%(실업급여 몫)이고
근로자는 그 절반씩 낸다. **국민연금만** 기준소득월액 상한(637만 원)과 하한(40만 원)이 걸린다.
요율은 해마다 바뀌므로 결과에 **기준 연도**를 함께 찍고, 바뀌면 `--pension 10` 처럼 총 요율을
직접 줄 수 있다.

**근로소득세는 계산하지 않는다.** 근로소득 간이세액표를 봐야 나오는 값이라 요율로 맞출 수
없다 — 그럴듯한 숫자를 만들어 내는 대신 4대보험까지만 세고, 급여명세서의 소득세+지방소득세를
`--tax` 로 주면 그것까지 빼서 실수령액을 낸다. 산재보험과 사업주의 고용안정·직업능력개발
몫도 세지 않는다(업종·규모마다 다르다). 공단 고지서는 10원 미만을 절사하는 곳이 있어 몇 원
다를 수 있다.
...
| `at novel check <파일\|폴더>` | 상투 표현, 군더더기 부사, 반복 어구, 같은 종결 어미 연속, 같은 말로 시작하는 문장 연속, 너무 긴 문장. 폴더면 화마다 |
| `at novel outline <경로…>` | 장면 목록 — 분량, 대사 비율, 등장인물, 첫 문장 (`-o` 로 xlsx 저장). 짧아서 뺀 덩어리 수도 알려 준다 |
...
| `at novel thread <경로…> -w <말>` | 복선·소재 추적 — 어느 화에 나오고 언제부터 안 나오는지. 심어 두고 안 쓴 소재를 짚는다 |
| `at novel pov <경로…>` | 시점·시제 흔들림 — 과거형 원고에 섞인 현재형 문장, 1인칭/3인칭이 다른 화 (대사는 빼고 지문만) |
| `at novel timeline <경로…>` | 시간 표현을 모아 보고, 한 장면 안에서 시간대·계절이 어긋난 곳을 짚는다 |
| `at novel find <찾을것> <경로…>` | 앞뒤 문장과 함께 찾고 처음·마지막 등장 위치를 알려준다 |
| `at novel names <경로…>` | 인물·지명 후보를 뽑아 표기 흔들림과 이름 뒤 조사 오류를 찾는다 (`민준`/`민준이` 는 한 사람으로 센다) |
...
| `at novel rename <옛이름> <새이름> <경로…>` | 인물 이름 바꾸기 - 뒤에 붙은 **조사까지** 맞춘다 (`--apply`) |
| `at novel quote <경로…>` | 따옴표 짝 점검 - 안 닫힌 대사, 굽은/곧은 따옴표 섞임 |
| `at novel punct <경로…>` | 문장 부호 점검·통일 - 줄임표(`...`→`……`), 줄표, 부호 앞뒤 빈칸 (`--apply`) |
...
at novel pov 원고/                 # 시제가 몇 문장만 벗어난 곳 (있으면 exit 1)
at novel pov 원고/03화.txt --each 20
at novel thread 원고/ -w 목걸이 -w 편지        # 심어 두고 안 쓴 소재
...
at novel rename 리안 세하 원고/ --apply     # 리안은 -> 세하는, 리안으로 -> 세하로
at novel punct 원고/                    # 줄임표·빈칸 점검 (미리보기)
at novel punct 원고/ --dash ― --apply   # 줄표까지 맞추고 실제로 고치기
...
`at novel thread` 는 복선·소재를 화별로 훑는다. `cast` 가 인물에 하는 일을 아무 낱말에나
한다 — 1화에 놓아둔 목걸이가 그 뒤로 여덟 화째 안 나오는지, 편지가 끝 무렵에야 처음 나오는지
같은 것을 표로 보여 준다. 조사가 붙은 꼴까지 세고 더 긴 낱말의 일부는 세지 않는다(`칼` 을
셀 때 `칼날` 은 빼고 센다). 끝에서 몇 화 안에 처음 나온 소재는 「이제 심은 것」이라 아무 말도
하지 않는다. **일부러 그렇게 둔 것일 수 있으므로 「이렇다」 까지만 적는다** — 고치라고 하지
않는다.

`at novel pov` 는 퇴고에서 가장 자주 지적받는 두 가지, **시제 혼용**과 **시점 흔들림**을
센다. 대사는 빼고 **지문만** 본다 — 대사 속 「나는」은 시점과 상관없다. 시제는 종결 어미로
가른다. `열었다·왔다`처럼 받침이 ㅆ 이면 과거, `한다·먹는다`처럼 ㄴ 이면 현재로 보고,
`있다·없다` 는 받침이 ㅆ 이지만 현재라 따로 뺀다. `그 남자.`, `차갑다.`, `그랬더라.` 처럼
어느 쪽인지 가릴 수 없는 문장은 **세지 않는다** — 애매한 것을 한쪽에 몰아넣으면 비율이
거짓말을 한다. 몇 개를 뺐는지는 함께 알려 준다.

주된 시제에서 **몇 문장만** 벗어난 화는 그 문장을 줄 번호와 함께 짚어 준다(있으면 종료
코드 1). 절반쯤 섞인 화는 짚지 않고 「섞어 씀」으로만 적는다 — 회상 장면처럼 일부러 바꾼
것일 수 있고, 그 판단은 쓰는 사람이 한다. 시점은 지문 문장 중 `나는·내가·내 …` 가 나온
비율로 보고, 다른 화와 다른 곳만 짚는다.

`at novel say` 는 대사만 뽑아 이어 놓는다. 지문을 걷어내고 읽으면 말투가 흔들리는 자리가
눈에 띈다. 화자는 `dialogue` 와 같은 규칙(같은 줄 → 옆 지문 한 줄)으로 찾고, 못 찾으면 비워
...
`at novel punct` 는 문장 부호를 맞춘다. 줄임표를 `...`, `. . .`, `…` 로 섞어 쓴 원고를
한 가지로(기본은 출판 관례인 `……`) 통일하고, 부호 뒤에 빈칸이 빠진 곳(`갔다.그리고`)과
부호 앞에 빈칸이 들어간 곳(`잤다 .`)을 고친다. **줄표는 `--dash` 를 줄 때만 손댄다** — 원고에
따라 `--` 를 다른 뜻으로 쓰기도 해서 기본으로 바꾸면 위험하다. 겹친 `??`·`!!` 는 **찾아
주기만 하고 고치지 않는다** — 힘을 주려고 일부러 쓴 것일 수 있고, 그 판단은 사람이 한다.
기본은 미리보기이고 `--apply` 로 고친 뒤에는 `at text undo` 로 되돌아온다.
...
"카일만 유독 반말인가", "두 인물의 말투가 구별되는가"를 숫자로 본다. 화자는 먼저 **같은 줄**
에서 찾고, 없으면 **바로 옆 지문 한 줄**까지만 본다 — `"오늘도 안 올 거야?"` 다음 줄에
`민준이 물었다` 만 오는 배치가 한국 소설에 흔해서, 같은 줄만 보면 대부분이 «못 찾음» 이
된다. 옆 줄은 조건이 까다롭다. 줄 전체가 아니라 **대사에 붙은 문장 하나**(다음 줄이면 첫 문장,
앞 줄이면 끝 문장)만 보고, 그 문장에 **이름이 하나만** 있고 `말했다`·`물었다` 같은 **말하는
동사**가 함께 있어야 한다. `민준이 물었다. 지수는 고개를 저었다` 에서 뒤 문장은 남의 반응이지
화자가 아니고, `리안과 카일이 함께 말했다` 처럼 한 문장에 이름이 둘이면 비워 둔다.
`at novel names` 의 «화자» 칸도 같은 규칙으로 센다 — 같은 원고에서 두 명령이 다른 사람을
화자로 내면 어느 쪽을 믿어야 할지 알 수 없다.
옆 줄에서 찾은 개수는 따로 알려 주고, 못 찾은 비율이 30%를 넘으면 그 사실도 알려 준다.
...
`로`가 맞다는 것(서울로)까지 본다. 여러 화를 한꺼번에 볼 때는 **어느 화의 몇 행인지**로
알려 준다(`02화.txt 5행`) — 이어 붙인 행 번호만 주면 사람이 그 줄을 찾을 수가 없다.
...
| `at ui 파일정리` | 한 번에 훑기·정리·사진 촬영일별·목록 만들기·중복 모으기·폴더 견주기·첨부 나눠 담기·백업 폴더에 넣기·문서에 남은 이름·사진에 남은 위치·이미지를 PDF 로·PDF 쪽 뽑기·PDF 쪽 번호·PDF 글자 꺼내기·받은 메일 훑기와 첨부 꺼내기·PDF 합치기 |
| `at ui 바꾸기` | 여러 파일의 글자·인코딩·줄바꿈을 한꺼번에 |
| `at ui 개인정보` | 보내기 전 폴더 훑기 - 주민번호·카드·계좌·여권·연락처가 든 파일 (읽기만 하고 값은 가려서 보여 준다) |
| `at ui 엑셀` | 일곱 탭 - 훑어보기(한 번에 훑기·점검·드문 값·빠진 것·합쳐 둔 칸 알림)·고치기(정리·표기 통일·빈 칸·날짜 열·마감일·나이·근무 시간과 임금·찾아 바꾸기)·골라내기(집계·그림)·여러 파일(서식 견주기·찾기·취합·메일 초안)·내보내기 전에(가리기·가릴 열 알아서 고르기·비슷한 값)·캘린더·연락처(ics·vcf)·인쇄(주소 라벨) |
| `at ui 문서` | 점검·용어·그림·목차·표 정렬·HTML/워드·한글·슬라이드→마크다운·여러 문서 합치기·회의록의 할 일·내보내기 |
| `at ui 원고` | 분량·되풀이·인물 등장 점검, 투고본(HTML·EPUB·워드)으로 내보내기 |
| `at ui 글자` | 자판 실수·표기 오류·줄 접기·NFC·표를 마크다운으로·연락처 뽑기·글자 수 세기 |
| `at ui 단축키` | 프로그램별 단축키를 나란히 놓고 찾는다 |
| `at ui 개발` | 주소 뜯어 보기·정규식·JWT·시각·cron·가리기·인코딩·키·.env·로그·sqlite·의존성 |
| `at ui json` | 응답 구조·두 응답 비교·표로·타입 코드 |
| `at ui 저장소` | 커밋 전 점검·시크릿·브랜치·충돌·TODO·통계·파일 이력·변경 로그 (읽기만) |
| `at ui 계산` | D-day·더치페이·대출·영업일·적금·전월세·근무시간·시급과 가산 수당·주휴수당·연차·퇴직금·단위·세금·4대보험 |
| `at ui 이럴땐` | 하고 싶은 일에서 명령을 찾는다 (터미널의 `at how` 와 같은 것, 그 일을 할 수 있는 화면으로 가는 길까지) |
| `at ui --list` | 화면 목록을 터미널에 찍는다 |
| `at ui --check` | 모든 화면을 브라우저로 열어 자바스크립트 오류를 본다 (오류 1, 못 열면 2). 느린 곳에서는 `--timeout 180` |
...
화면이 열네 개라 런처 맨 위에 **찾기 상자**를 두었다. «취합», «첨부», «연차» 처럼 하고 싶은 일을
적으면 그 화면만 남는다(화면 이름·별명·한 줄 설명을 함께 본다). 그래도 없으면 터미널에서
`at how <하고싶은일>` 로 명령 줄까지 보거나 `at find <말>` 로 명령을 찾는다.
...
`mobile/` 은 갤럭시·아이폰 홈 화면에 **앱처럼 까는** 한 화면짜리 도구다.
브라우저에서 돌 수 있는 것은 도구로 옮기고, 못 도는 것은 명령만 담았다.

| 탭 | 하는 일 |
| --- | --- |
| 도구 | 폰에서 진짜 도는 도구 21가지 — 계산 17, 글 3, 표 1. 검색으로 찾는다 |
| 장부 | 수입·지출 적기, 분류별과 지난달 대비, 되풀이된 지출 찾기 (`at sheet ledger`) |
| 고정비 | 구독·할부를 한 달로 환산, 1년치, 곧 끝나는 것 (`at sheet budget`) |
| 더보기 | csv·json 내보내기와 되살리기 |

폴더 정리·git·서버처럼 브라우저가 못 하는 것은 없는 척하지 않고 **명령 찾기**에
...
기록은 그 폰의 브라우저에만 남는다. 서버로 보내지 않는다. 내보낸 csv 는
`at sheet ledger` 가 그대로 읽는 꼴이라, 폰에서 적고 PC 에서 집계하는 식으로
이어 쓴다. 자세한 것은 [mobile/README.md](mobile/README.md).

셈법은 터미널 쪽과 같은 수를 쓴다. `tests/test_mobile.py` 가 최저임금·보험
요율은 물론 **끼워 둔 명령 목록이 지금 파서와 같은지**까지 본다 — 명령을 더하고
`python3 mobile/tools/make_data.py` 를 안 돌리면 거기서 걸린다.
...
```bash
python3 build.py                      # dist/ 에 at.pyz + at.bat + 업데이트.bat
python3 dist/at.pyz file sweep ~/다운로드
python3 dist/at.pyz ui                 # 브라우저 화면도 그대로 된다
```

**윈도우에서는 `at.bat` 을 `at.pyz` 옆에 두고 더블클릭**하면 된다 — 터미널을 몰라도
브라우저 화면(`at ui`)이 뜬다. 폴더나 파일을 `at.bat` 위로 **끌어다 놓으면** 그것으로
할 만한 명령을 알려 준다. 파이썬이 없으면 어디서 받아 무엇을 켜야 하는지 한국어로
알려 준다(설치 화면의 «Add python.exe to PATH»). 이 두 `.bat` 은 **윈도우에서 직접
시험해 보지 못했다** — 이 저장소의 시험은 리눅스에서 도는 것까지만 확인한다.

새 기능은 이렇게 받는다. `git` 으로 받았으면 `git pull` 이면 그 자리에서 최신이 되고
(`업데이트.bat` 이 같은 일을 한다), `.pyz` 로 받았으면 **새 `at.pyz` 로 덮어쓰면 된다** —
지울 것은 없고, 설정·되돌리기 기록은 `~/.attools` 에 따로 있어 그대로 남는다.
스스로 인터넷에서 받아 자기를 덮어쓰는 기능은 넣지 않았다 — 서명 없는 코드를 자동으로
갈아 끼우는 것이라, 그 편의보다 위험이 크다.
