...
읽는 쪽도 남이 만든 파일을 넉넉히 받아들인다. 칸 주소(`r="B2"`)를 적지 않는 도구가 있는데,
그때는 **나온 차례를 열 자리로** 본다 - A 열로 몰아 넣으면 앞 칸이 조용히 사라진다. 인라인
문자열(`inlineStr`)과 서식이 섞인 글자도 이어 붙여 읽고, 수식 셀은 저장된 계산 결과를 읽는다
