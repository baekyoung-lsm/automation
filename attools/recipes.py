...
RECIPES: list[Recipe] = [
    ...
    Recipe("계산", "카드·통장 내역으로 지난달 얼마 썼는지 보기",
           ["at sheet ledger 카드내역.csv --date 날짜 --amount 금액 "
            "--name 가맹점 --group 분류",
            "at sheet ledger 통장.csv --date 거래일 --name 적요 "
            "--in 입금액 --out 출금액"],
           "달마다 수입·지출, 분류별 지난달 대비, 큰 지출을 낸다. 달마다 "
           "되풀이된 결제도 찾아 준다 - 잊고 있던 구독이 여기서 걸린다. "
           "앞으로 매달 얼마 나갈지는 at sheet budget 이다.",
           "가계부 카드내역 통장 거래내역 지출 결산 구독 찾기"),
