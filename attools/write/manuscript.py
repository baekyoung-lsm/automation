...
def export_docx(chapters: list[tuple[str, str]], dest: "Path", *, title: str = "",
    ...
    parts: list[str] = []
    if title:
        parts.append(docx.paragraph(title, size=36, bold=True, center=True))
    if author:
        parts.append(docx.paragraph(author, size=22, center=True))
    if note:
        parts.append(docx.paragraph(note, size=18, center=True))

    for number, (name, body) in enumerate(chapters):
        parts.append(docx.paragraph(name, size=28, bold=True,
        ...
        for block in body.split("\n\n"):
            ...
            if not block:
                continue
            if SEPARATOR_ONLY.match(block) or block in ("＊", "*"):
                parts.append(docx.paragraph("＊ ＊ ＊", center=True))
                continue
            for line in block.splitlines():
                if line.strip():
                    parts.append(docx.paragraph(line.strip(), first_line=indent))
    return docx.write_document(dest, parts)
